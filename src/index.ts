...
async function getFeedContent({
    ...
    try {
        ...
        const createPromises = feedContents.map(
            async (content): Promise<IResultValue | string> => {
                const entity = {
                    title: content.title || '',
                    link: content.content || '',
                };
                const existItem = await subscriptionService.findByTitleAndLink(
                    entity,
                );
                if (!existItem) {
...
readFeeds('Feeds.xml').then(async (feedObjects: IFeedObject[]) => {
    ...
    const allResult = await Promise.allSettled(
        filterResult.map(async (obj) => await getFeedContent(obj)),
    );
    console.log('=====================');
    // 总计执行
    ...
    console.log(
        `成功：${solution.successCount}，失败：${solution.failCount}，总计：${total}`,
    );
